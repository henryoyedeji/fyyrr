...
def edit_artist_submission(artist_id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            flash('Failed to update Artist {}: {}'.format(artist.name, e))
        finally:
            db.session.close()
...
def search_shows():
    ...
    shows = Show.query.join(Artist, Show.artist_id == Artist.id).join(Venue, Show.venue_id == Venue.id)\
        .filter(or_(Artist.name.ilike(f'%{search_term}%'), Venue.name.ilike(f'%{search_term}%'))).all()
    # data = show_artist.query.filter(or_(artist.name.ilike(f'%{search_term}%'),
    #                              Show.venue.name.ilike(f'%{search_term}%'))).all()
    data = []
    for show in shows:
        data.append({
            "artist_image_link": show.artist.image_link,
...
def create_show_submission():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            # TODO: on unsuccessful db insert, flash an error instead.
            flash('An error occurred. Show could not be listed. - {}'.format(e))
            # see: http://flask.pocoo.org/docs/1.0/patterns/flashing/
    return render_template('pages/home.html')
